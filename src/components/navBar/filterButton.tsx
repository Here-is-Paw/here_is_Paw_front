    ...
    <div className="flex flex-wrap items-center gap-1 flex-auto justify-end">
      ...
      <Button
        ...
        onClick={() => {
          ...
          console.log(
            `발견했개 마커 ${getVisibilityText(buttonStates.found)} 상태입니다.`
          );
          toggleButton("found");
          console.log(
            `발견했개 버튼 상태 변경됨. 변경 후 상태는 ${!buttonStates.found}이 될 예정`
          );
          console.log(
            `발견했개 마커가 이제 ${getVisibilityText(
              !buttonStates.found
            )} 상태로 변경됩니다.`
          );
        }}
      ...
      <Button
        ...
        onClick={() => {
          console.log(
            `동물보호센터 버튼 클릭됨. 현재 상태: ${buttonStates.hospital}`
          );
          console.log(
            `동물보호센터 마커 ${getVisibilityText(
              buttonStates.hospital
            )} 상태입니다.`
          );
          toggleButton("hospital");
          console.log(
            `동물보호센터 버튼 상태 변경됨. 변경 후 상태는 ${!buttonStates.hospital}이 될 예정`
          );
          console.log(
            `동물보호센터 마커가 이제 ${getVisibilityText(
              !buttonStates.hospital
            )} 상태로 변경됩니다.`
          );
        }}
