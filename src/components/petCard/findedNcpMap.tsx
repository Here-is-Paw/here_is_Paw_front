...
const GetFindLocationPicker = ({ onLocationSelect, initialLocation }: LocationPickerProps) => {
  const mapElement = useRef<HTMLDivElement>(null);
  const mapInstance = useRef<naver.maps.Map | null>(null);
  const markerRef = useRef<naver.maps.Marker | null>(null);
  const [selectedLocation, setSelectedLocation] = useState<string>(initialLocation?.location || "");
      ...
      naver.maps.Service.reverseGeocode(
        ...
        (status, response) => {
          ...
          if (response.v2.results && response.v2.results.length > 0) {
            ...
            // 지도 클릭 또는 마커 드래그 시 selectedLocation 업데이트
...
export default GetFindLocationPicker;
