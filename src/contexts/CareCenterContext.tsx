...
// Context 생성
const CareCenterContext = createContext<CareCenterContext | undefined>(
  undefined
);
...
export const CareCenterProvider: React.FC<{ children: ReactNode }> = ({
  ...
  const convertResponseToCareCenter = (
    results: CareSearchResponse[]
  ): CareCenter[] => {
  ...
  const loadRadiusData = async (keyword = "") => {
    try {
      ...
      interface RadiusCenterResponse {
        id: number;
        careNm: string;
        careAddr: string;
        careTel: string;
      ...
      const mappedCenters: CareCenter[] = (
        radiusCenters as RadiusCenterResponse[]
      ).map((center) => ({
        id: center.id,
        name: center.careNm,
        address: center.careAddr,
        phoneNumber: center.careTel,
        operatingHours: `${center.weekOprStime || ""}-${
          center.weekOprEtime || ""
        }`,
  ...
  return (
    <CareCenterContext.Provider value={value}>
      {children}
...
export function useCareCenterContext() {
  ...
  if (context === undefined) {
    throw new Error(
      "useCareCenterContext must be used within a CareCenterProvider"
    );
  }
  return context;
}
