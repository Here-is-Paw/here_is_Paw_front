// 반경에 따른 최적의 줌 레벨 계산 함수
const getOptimalZoomForRadius = (radius: number) => {
  if (radius <= 500) return 17;      // 500m: 가까운 거리
  if (radius <= 1000) return 16;     // 1km 이하: 동네 반경
  if (radius <= 2000) return 15;     // 2km 이하: 좀 더 넓은 반경
  if (radius <= 3000) return 14;     // 3km 이하
  if (radius <= 4000) return 13;     // 4km 이하
  return 12;                         // 5km: 넓은 지역
...
const NcpMap = ({ currentLocation, lostPets, findPets }: NcpMapProps) => {
  ...
  const isWithinRadius = (petLat: number, petLng: number, centerLat: number, centerLng: number, radius: number) => {
    ...
    const petPosition = new window.naver.maps.LatLng(petLat, petLng);
  ...
  useEffect(() => {
    if (circleRef.current && mapInstance.current && currentLocation.coordinates) {
      ...
      // 원이 지도에 보이게 하기 위해 필요한 경우만 확대 수준 조정 (선택적)
      const currentZoom = mapInstance.current.getZoom();
      const optimalZoom = getOptimalZoomForRadius(radius);

      // 현재 확대 수준이 너무 넓어서 원이 잘 안 보일 경우에만 확대 수준 조정
      if (currentZoom < optimalZoom - 2) {
        mapInstance.current.setZoom(optimalZoom);
      }
    }
  }, [radius]);
  ...
  useEffect(() => {
    ...
    const initializeMap = () => {
      ...
      try {
        ...
        // 초기 확대 수준은 반경에 맞게 설정
        const initialZoom = getOptimalZoomForRadius(radius);
